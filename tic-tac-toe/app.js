...
let ticTakToe = {
     ...
     * @param {Event} event 
     * @returns {boolean} Вернет true в случае если статус игры "играем", клик что описан 
     * в объекте event был по ячейке и ячейка, куда был произведен клик, был по пустой ячейке.
     */
    ...
    /**
     * Заполяняем ячейку в которую кликнул пользователь в событии event.
     * @param {Event} event 
     * @param {HTMLElement} event.target
     */
    fillCell(event) {
        // Получаем стороку и колонку куда кликнули.
        let row = +event.target.dataset.row;
        let col = +event.target.dataset.col;

        // Заполняем ячейку и ставим значение в массиве, в свойство mapValues.
    ...
    hasWon() {
        ...
        this.isLineWon({x: 2, y:0}, {x: 2, y:1}, {x: 2, y:0}) ||
    ...
    /**
     * Приверка если ли выиграшная ситуация на линии.
